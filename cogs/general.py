...
class General(commands.Cog):
    ...
    async def serverinfo(self, ctx: commands.Context):
        ...
        embed.set_footer(text=f"Requested by {ctx.author if isinstance(ctx, commands.Context) else ctx.user}", 
                        icon_url=(ctx.author if isinstance(ctx, commands.Context) else ctx.user).display_avatar.url)
    ...
    async def setprefix(self, ctx: commands.Context, prefix: str):
        ...
        embed.set_footer(text=f"Changed by {ctx.author if isinstance(ctx, commands.Context) else ctx.user}", 
                        icon_url=(ctx.author if isinstance(ctx, commands.Context) else ctx.user).display_avatar.url)
